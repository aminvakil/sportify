...
2. Defer `symfony/swiftmailer-bundle`/`swiftmailer/swiftmailer` replacement until the app is on a Symfony version that can install `symfony/mailer`.
...
2. Upgrade dependency blockers before the next Symfony major bump:
   - Replace or remove packages that do not have a viable Symfony 5.4+ path, especially FOSOAuthServerBundle, FOSRestBundle/JMS/Hateoas/Nelmio API docs, DoctrineBundle/cache, and Swiftmailer integration.
